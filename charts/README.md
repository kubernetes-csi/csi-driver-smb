...
| `linux.krb5AutoConfig`                                  | when true (and `krb5CacheDirectory` is non-empty), an initContainer drops `/etc/krb5.conf.d/90-csi-driver-smb.conf` on every Linux node so `cifs.upcall` finds the per-uid credential cache produced by the driver | `false`                                                 |
| `linux.krb5AutoConfigImage`                             | image used by the krb5 auto-config initContainer                                                           | `busybox:stable`                                        |

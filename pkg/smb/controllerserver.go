...
import (
	...
	volumehelper "github.com/csi-driver/csi-driver-smb/pkg/util"
...
// CreateVolume provisions an smb
...
func (d *Driver) DeleteVolume(ctx context.Context, req *csi.DeleteVolumeRequest) (*csi.DeleteVolumeResponse, error) {
	...
	klog.V(2).Infof("smb(%s) under rg(%s) account(%s) volume(%s) is deleted successfully", fileShareName, resourceGroupName, accountName, volumeID)
	...
	if exists {
		...
		return &csi.CreateSnapshotResponse{
			Snapshot: &csi.Snapshot{
				...
				CreationTime:   tp,
				// Since the snapshot of smb has no field of ReadyToUse, here ReadyToUse is always set to true.
				ReadyToUse: true,
			},
		}, nil
	}
